...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  const { businessId, mode, productUrls, enrich } = req.body as {
    businessId?: string
    mode?: ScrapeMode
    productUrls?: string[]
    enrich?: boolean
  ...
  if (resolvedMode === 'products' && (!productUrls || productUrls.length === 0)) {
  ...
  try {
    const urls = resolvedMode === 'products' ? (productUrls ?? []) : [business.url]
    const { upserted, errors, enriched, diff } = await scrapeAndUpsert(
      {
